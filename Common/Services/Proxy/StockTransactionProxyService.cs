...
{
    public class StockTransactionProxyService(HttpClient httpClient, IOptions<JsonOptions> jsonOptions) : ITransactionService
    {
        ...
        {
            var response = await _httpClient.PostAsJsonAsync("api/StockTransaction", transaction, _jsonOptions);
        ...
        {
            var response = await _httpClient.DeleteAsync($"api/StockTransaction/{id}");
            return response.IsSuccessStatusCode;
        }

        public async Task<List<StockTransaction>> GetAllTransactionsAsync()
        {
            var transactions = await _httpClient.GetFromJsonAsync<IEnumerable<StockTransaction>>("api/StockTransaction", _jsonOptions) ?? [];
            return [.. transactions];
        }

        public async Task<StockTransaction> GetTransactionByIdAsync(int id)
        {
            return await _httpClient.GetFromJsonAsync<StockTransaction>($"api/StockTransaction/{id}", _jsonOptions) ?? throw new InvalidOperationException("Transaction not found");
        }

        public async Task<StockTransaction> UpdateTransactionAsync(int id, StockTransaction transaction)
        {
            var response = await _httpClient.PutAsJsonAsync($"api/StockTransaction/{id}", transaction, _jsonOptions);
            response.EnsureSuccessStatusCode();
            return await response.Content.ReadFromJsonAsync<StockTransaction>(_jsonOptions) ?? throw new InvalidOperationException("Failed to deserialize response");
        }
        ...
        {
            var transactions = await _httpClient.GetFromJsonAsync<IEnumerable<StockTransaction>>($"api/StockTransaction/user/{userId}", _jsonOptions) ?? [];
            return [.. transactions];
        }
        ...
        {
            var url = $"api/StockTransaction/filter?searchTerm={searchTerm}&sortBy={sortBy}&ascending={ascending}";
            if (userId != null)
            {
                url += $"&userId={userId}";
            }
        ...
        {
            var response = await _httpClient.PostAsJsonAsync("api/StockTransaction/filter", criteria, _jsonOptions);
            response.EnsureSuccessStatusCode();
